...
export const meta = {
  ...
  category:    'ai',
  guide:       ['<your message>', 'Reply to any message with "copilot" (no text needed)'],
  cooldowns:   5,
};
...
export async function onStart({ event, args, response, usage }) {
  ...
  try {
    ...
    return data?.answer
      ? response.reply(data.answer)
      : response.reply('🔴 No response received from the AI.');
  } catch (error) {
    return response.reply(`🔴 API Error: ${error.message || 'Failed to connect to Ajiro API'}`);
  }
}
