...
export async function onEvent({ api, event, Users, Threads, response }) {
  ...
  if (isSelf) {
    try {
      await response.call('addUserToGroup', leftID, threadID);
      await response.send(`🔒 ${name} tried to leave but was re-added. Anti-Out is active!`, threadID);
    } catch {
      await response.send(`❌ Unable to re-add ${name}. They may have blocked the bot.`, threadID);
    }
  }
}
